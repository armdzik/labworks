...
import edu.ktu.ds.lab3.Dzikas.ParsableMapOa;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private void mapKeys()
    {
      ...
      KsGui.oun(taEvents, key, MESSAGES.getString("mapKeys"));
    }
    
    private void numberOfEmpties()
    {
        KsGui.oun2(taEvents, " ", (MESSAGES.getString("mapEmptyPlace") + map.numberOfEmpties() + " "));
    }
    
    private void containsValue()
    {       
         Book k = new Book(); //Aristotelis Valstybė 2006 62 800.0
         k.parse(taInput.getText());
        KsGui.oun2(taEvents, " ", (MESSAGES.getString("mapContains") + map.containsValue(k) + " "));
    }

    private void mapGeneration(String filePath) {
        // Išjungiami 2, 4, 5 ir 6 mygtukai
        IntStream.of(1, 3, 4, 5).forEach(p -> paneButtons.getButtons().get(p).setDisable(true));
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            ...
            case 3:
                map = new ParsableMapOa<>(String::new, Book::new, initialCapacity, loadFactor, ht);
                break;
            default:
                IntStream.of(1, 3, 4, 5).forEach(p -> paneButtons.getButtons().get(p).setDisable(true));
                taInput.setDisable(true);
