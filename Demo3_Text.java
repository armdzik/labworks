...
public class Demo3_Text extends BaseConsole{
    ...
    private void sumInts() { // String skaidome į skaičius ir sumuojame
        ...
        String[] sa = readLastLine().split("(\\s+)"); // " +" //Jau padaryta - ?
        int sum = 0;
        for(String s: sa)
    ...
    private void sortStandart(){
        ...
        Arrays.sort(ws);  // Rikiavimas be Collator - visos lietuviškos gale //nebūtinai
    ...
    private void sortLT(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText(String.join(nL, words) + nL);
        String[] ws = ta1.getText().split(nL);
        Collator collatorLT = Collator.getInstance(new Locale("LT")); //Rikiuoja Lietuviškai
...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    ...
    ***************************************************/
    void regexExamples(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText(" -žodis(word) naujas žodis" + nL);
        String[] sa = readLastLine().split("-");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa)+nL); // pakartojame dešinėje //Visus sujoinina kiek yra masyve
        Pattern pat = Pattern.compile("[ėęžųūį\\w]+|[ėęžųūį\\w]+");      // pirma dalis - tai Pattern(modelis, šablonas) ką reikia surasti
        Matcher m = pat.matcher(sa[1]);            // kita dalis - apdorojimui //idedama tai iš kur bus ieškoma
