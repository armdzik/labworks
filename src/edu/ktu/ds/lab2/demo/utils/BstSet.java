 ...
 */
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable, Iterable<E>{
    ...
    public boolean removeAll(BstSet<? extends E> c)//-----------------------------------------------Visų elementų pašalinimas--------------||
    {
     ...
     public boolean retainAll(BstSet<E> c){
      ...
      public E last() {
        BstNode<E> node = root;
        while (node != null) {
            if (node.right.element.equals(node.left.element)) {
                node = node.right;
                if (node.right.element.equals(node.left.element) == false) {
